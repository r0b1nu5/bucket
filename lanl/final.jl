...
"""
	run_l1(Xs::Array{Float64,2}, tau::Float64, ks::Tuple{Int64,Int64,Int64}, is_laplacian::Bool, plot::Bool=false, b::Float64=0., mu::Float64=1e-1, bp::Float64=1e-1)
...
`(L_l1, A_l1, d_l1, gamma_l1, k_l1, l_l1)`: results under l1.
	`L_l1`: Minimal value of the objective.
	`A_l1`: Estimate of the dynamics matrix.
	`d_l1`: Estimate of the damings.
	`gamma_l1`: Estimate of the forcing amplitude.
	`k_l1`: Estimate frequency index (see theory).
	`l_l1`: Estimate of the forcing location.
"""
function run_l1(Xs::Array{Float64,2}, tau::Float64, ks::Tuple{Int64,Int64,Int64}, is_laplacian::Bool, plot::Bool=false, b::Tuple{Float64,Float64}=(0.,0.), mu::Float64=1e-1, bp::Float64=1e-1)
	nn,NN = size(Xs)
	n = Int(nn/2)
	N = NN-1
...
# Run the optimizations
	Ls_l1 = zeros(length(kmin:dk:kmax))
	L_l1 = 1000.
	A_l1 = zeros(n,nn)
	d_l1 = zeros(n)
	gamma_l1 = zeros(n)
	...
	for k in kmin:dk:kmax
		...
		@info "l1: k = $k"
		if is_laplacian
			Lt = Lmin_l1_lap(x,Dx,xt,Dxt,k,A1h,a2h,b,mu,bp)
		else
			Lt = Lmin_l1(x,Dx,xt,Dxt,k,A1h,a2h,b,mu,bp)
		end
		Ls_l1[c] = Lt[1]
		if Lt[1] < L_l1
			L_l1,A_l1,d_l1,gamma_l1 = Lt
			k_l1 = k
			l_l1 = findmax(gamma_l1)[2]
		end
	end

	@info "Best l1: ω = $(2*pi*(k_l1)/(tau*N)), L = $(L_l1)."

	if plot
		T = N*tau
		figure("l1, T = $(N*tau)")
		subplot(1,2,1)
		plot_l1_1(Ls_l1, L_l1, gamma_l1, l_l1, k_l1, ks, T)
		subplot(1,2,2)
		plot_l1_2(Ls_l1, L_l1, gamma_l1, l_l1)
	end

	return Ls_l1, (L_l1,A_l1,d_l1,gamma_l1,l_l1,k_l1)
end
...
"""
	Lmin_l1(x::Array{Float64,2}, Dx::Array{Float64,2}, xt::Array{Complex{Float64},2}, Dxt::Array{Complex{Float64,2}, k::Int64, A1h::Array{Float64,2}, a2h::Array{Float64,1}, b::Float64=0., mu::Float64=1e-1, bp::Float64=1e-1)
...
"""
function Lmin_l1(x::Array{Float64,2}, Dx::Array{Float64,2}, xt::Array{Complex{Float64},2}, Dxt::Array{Complex{Float64},2}, k::Int64, A1h::Array{Float64,2}, a2h::Array{Float64,1}, b::Tuple{Float64,Float64}=(0.,0.), mu::Float64=1e-1, bp::Float64=1e-1)
	t0 = time()
...
# (f_l(k))^T*A_{l,:}
	...
	@NLobjective(system_id, Min, T1 + 2*T2 + .5*g2 - 2/sqrt(N)*sum(gamma[l]*sqrt(T3[l] + 2*T4[l] + glk[l]) for l = 1:n) + b1*sum(abs(A1[i,j]) for i = 1:n-1 for j = i+1:n) + b2*sum(gamma[l] for l in 1:n))
...
"""
	objective_l1(Xs::Array{Float64,2}, tau::Float64, A1::Array{Float64,2}, d::Array{Float64,1}, gamma::Array{Float64,1}, l::Int64, k::Int64)
...
`obj`: Value of the objective.
"""
function objective_l1(Xs::Array{Float64,2}, tau::Float64, A1::Array{Float64,2}, a2::Array{Float64,1}, gamma::Array{Float64,1}, k::Int64, b::Float64=0.)
	x = Xs[:,1:end-1]
	nn,N = size(x)
	n = Int(nn/2)
	...
	for l in 1:n
		if gamma[l] > 0.
			temp += gamma[l]*sqrt(tr(A[l,:]*transpose(A[l,:])*Fk) + (2*flk[l]*A[l,:])[1] + glk[l])
		end
	end

	obj = tr(transpose(A)*A*Sigma0) + 2*tr(A*Sigma1) + .5*sum(gamma.^2) - 2/sqrt(N)*temp + b*sum(abs(A1[i,j]) for i = 1:n-1 for j = i+1:n)
...
"""
	plot_l1_1(Ls_l1::Array{Float64,1}, L_l1::Float64, gamma_l1::Array{Float64,1}, l_l1::Int64, k_l0::Int64, ks::Tuple{Int64,Int64,Int64}, T::Union{Float64,Int64})
...
`gamma_l1`: Corresponding forcing amplitudes.
`l_l1`: Index with largest amplitude.
`k_l1`: Corresponding value of k.
`ks = (kmin,kmax,dk)`: Values of assessed (kmin:dk:kmax).
"""
function plot_l1_1(Ls_l1::Array{Float64,1}, L_l1::Float64, gamma_l1::Array{Float64,1}, l_l1::Int64, k_l1::Int64, ks::Tuple{Int64,Int64,Int64}, T::Union{Float64,Int64})
	kmin,kmax,dk = ks
	ws = 2*pi*(kmin:dk:kmax)/T
	PyPlot.plot(ws,Ls_l1,"o")
	xlabel("ω")
	ylabel("Log-likelihood")
	title("Best l1: ω = $(round(2*pi*k_l1/T,sigdigits=5)), L = $(round(L_l1,sigdigits=5))")
end

"""
	plot_l1_2(Ls_l1::Array{Float64,1}, L_l1::Float64, gamma_l1::Array{Float64,1}, l_l1::Int64)
...
`l_l1`: Index with largest amplitude.
"""
function plot_l1_2(Ls_l1::Array{Float64,1}, L_l1::Float64, gamma_l1::Array{Float64,1}, l_l1::Int64)
	PyPlot.plot(1:length(gamma_l1),gamma_l1,"o")
	xlabel("node index")
	ylabel("Estimated amplitude")
	title("Source: l = $(l_l1), γ_l = $(round(gamma_l1[l_l1],sigdigits=4))")
end
...
"""
	Lmin_l1_lap(x::Array{Float64,2}, Dx::Array{Float64,2}, xt::Array{Complex{Float64},2}, Dxt::Array{Complex{Float64,2}, k::Int64, A1h::Array{Float64,2}, a1h::Array{Float64,1}, b::Float64=0., mu::Float64=1e-1, bp::Float64=1e-1)

Same as Lmin_l1, but assumes that the dynamics matrix (A1) is a Laplacian, with nonpositive off-diagonal terms.
"""

function Lmin_l1_lap(x::Array{Float64,2}, Dx::Array{Float64,2}, xt::Array{Complex{Float64},2}, Dxt::Array{Complex{Float64},2}, k::Int64, A1h::Array{Float64,2}, a2h::Array{Float64,1}, b::Float64=.01, mu::Float64=1e-1, bp::Float64=1e-1)
	t0 = time()
