def get_partition(list_meter: list[str], list_pe: list[list[str]], list_fs: list[list[str]]) -> list[list[str]]:
    ...
    for fs in list_fs:                                  # Loop over all siblings pairs.
        if (fs[0] in list_p and [fs[0],fs[1]] not in list_pe) or (fs[1] in list_p and [fs[1],fs[0]] not in list_pe):          # If either of the siblings is a parent but the pair is not a parent-child pair, then do nothing.
            continue
...
# The goal is to attribute a unique parent to each siblings cluster.
    ...
    for part in partition:          # For each siblings cluster...
        ...
        pi = list(set(ps[0]).intersection(*ps[1:]) - set(part))     # Extract the common parents and remove the siblings that may be in the parents list.
        pu = list(set().union(*ps) - set(part))                     # Extract the union of parents and remove the siblings that may be in the parents list.
        partt = []
