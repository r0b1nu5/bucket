...
for r in rs:
    with open(f"data-robin/P_{r}_{i1}_{i2}.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(head)
    with open(f"data-robin/Q_{r}_{i1}_{i2}.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(head)
    with open(f"data-robin/V_{r}_{i1}_{i2}.csv", 'w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(head)
    ...
    for i in range(10000):
        ...
        for i in range(len(mup)):
            if i == i1 or i == i2:
                ...
                p = mup.loc[i].values[0] + sigp.loc[i].values[0]*np.random.normal()
                q = muq.loc[i].values[0] + sigq.loc[i].values[0]*np.random.normal()
        ...
        with open(f"data-robin/P_{r}_{i1}_{i2}.csv", 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(ps)
        with open(f"data-robin/Q_{r}_{i1}_{i2}.csv", 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(qs)
        with open(f"data-robin/V_{r}_{i1}_{i2}.csv", 'a', newline='') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(vs)
