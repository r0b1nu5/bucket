...
for i in 1:length(Tts)
	...
	push!(Tbs,readdlm("data_cluster/Tb$(i)_vs_Tt.csv",',')[1])
	push!(Wouts,readdlm("data_cluster/Wout$(i)_vs_Tt.csv",','))
...
for d in [1,2,5,10]
	subplot(3,1,2)
	PyPlot.semilogy(Tts[1:end-d],norm.(Wouts[1+d:end]-Wouts[1:end-d])./norm.(Wouts[1:end-d]),"-o",label="δT = $(10*d)")
end
...
for i in 1:length(Ns)
	Tb = Array{Float64,1}()
	for k in 1:50
		push!(Tb,readdlm("data_cluster/Tb$(k).$(i)_vs_N_Tt1001.csv",',')[1])
	end
