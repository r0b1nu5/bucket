...
rank2 = Array{Int64,1}(sortslices([dist 1:m],dims=1,rev=true)[:,2])
...
while run2 && count < m - n + 1
	...
	rank2 = Array{Int64,1}(sortslices([dist setdiff(1:m,rmvd2)],dims=1,rev=true)[:,2])
	push!(ranks2,rank2)
...
writedlm("data/ieee57_ranks_updated_rank_$(P0)_rev.csv",ranks2,',')
writedlm("data/ieee57_rmvd_updated_rank_$(P0)_rev.csv",rmvd2,',')
writedlm("data/ieee57_cuts_updated_rank_$(P0)_rev.csv",cuts2,',')
