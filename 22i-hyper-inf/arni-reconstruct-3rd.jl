...
include("arni-basis-expansion-3rd.jl")

function reconstruct_3rd(X, DX, NODE, connectivity, th=1e-4, BASIS="polynomial_diff", ORDER=3, MODEL="kuramoto1")
...
=#
            # Beginning of reconstruction algorithm
            println("Performing ARNI...")
	    Y,table = basis_expansion_3rd(X, ORDER, BASIS, NODE)
	    nolist = 1:Int64(N*(N-1)/2)
            list = Int64[]
            cost = Float64[]
            b = true
	    vec = zeros(Int64(N*(N-1)/2))
            while !isempty(nolist) && b
                # Composition of inferred subspaces
                Z = zeros(0, T)
                for n in list
