...
function generate_countries(n_real::Int64, NS::Int64, N::Array{Int64,1}, M::Array{Int64,1}, MU::Array{Float64,1}, SI::Array{Float64,1}, DE::Array{Float64,1}, epss::Array{Float64,1})
	emi = minimum(epss)
	X0s = Array{Array{Array{Float64,1},1},1}()	# Natural opinions in each state
	for r in 1:n_real
		X0 = Array{Array{Float64,1},1}()
		for i in 1:NS
			maxd = 1000.
			while maxd > emi
				x = big_rand(N[i],-MU[i]+DE[i],SI[i],MU[i]+DE[i],SI[i])
	...
	X1s = Array{Array{Array{Float64,1},1},1}()
	state_id_s = Array{Array{Int64,1},1}()
	LDis_s = Array{Array{Array{Float64,2},1},1}()
	for r in 1:n_real
		...
		push!(X1s,X1)
		push!(state_id_s,state_id)
		push!(LDis_s,LDis)
	end

	return X0s, X1s, LDis_s, state_id_s
end
