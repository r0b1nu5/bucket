...
figure("$run",(11.,5.))
subplot(1,2,1)
PyPlot.fill([αs[id];αs[di]],[β2[1,id];zeros(T)],"k",alpha=.1)
PyPlot.plot(αs[id],β2[1,id],"-",color=col1,linewidth=lw,label=runs[run]["labs"][2])
#PyPlot.plot(αs[id],-β1[1,id],"--",color=col1,linewidth=lw)
PyPlot.fill([αs[id];αs[di]],[β0[1,id];zeros(T)],"k",alpha=.1)
PyPlot.plot(αs[id],β0[1,id],"-",color=col2,linewidth=lw,label=runs[run]["labs"][0])
#PyPlot.plot(αs[id],-β0[1,id],"--",color=col2,linewidth=lw)
PyPlot.fill([αs[id];αs[di]],[β1[1,id];zeros(T)],"k",alpha=.1)
PyPlot.plot(αs[id],β1[1,id],"-",color=col3,linewidth=lw,label=runs[run]["labs"][1])
