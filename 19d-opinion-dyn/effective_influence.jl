using Plot, SpecialFunctions

function influence_balance(x::Float64, ϵ::Float64, p::Float64=.5, Δ::Float64=0., μ::Float64=0., σ::Float64=.2)
	left = p/2*(erf((x-μ-Δ)/(sqrt(2)*σ)) - erf((x-ϵ-μ-Δ)/(sqrt(2)*σ))) + (1-p)/2*(erf((x-μ+Δ)/(sqrt(2)*σ)) - erf((x-ϵ-μ+Δ)/(sqrt(2)*σ)))
	right = p/2*(erf((x+ϵ-μ-Δ)/(sqrt(2)*σ)) - erf((x-μ-Δ)/(sqrt(2)*σ))) + (1-p)/2*(erf((x+ϵ-μ+Δ)/(sqrt(2)*σ)) - erf((x-μ+Δ)/(sqrt(2)*σ)))
...
function influence_balance(xs::Vector{Float64}, ϵs::Vector{Float64}, p::Float64=.5, Δ::Float64=0., μ::Float64=0., σ::Float64=.2)
	L = zeros(length(xs[:,1]),0)
	R = zeros(length(xs[:,1]),0)
	for e in ϵs
