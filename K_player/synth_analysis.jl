...
data_list = ["synth02_200_10000_70",]
n100_sample = 1

for j in data_list
	for id in 1:n100_sample
		push!(tups,(j,100,true,true,true,true,false,false,id))
	end
end

function synth_analysis_parallel(tup::Tuple{String,Int64,Bool,Bool,Bool,Bool,Bool,Bool,Int64})
	j = tup[1]
	number_sample = tup[2]
	save = tup[3]
	pl = tup[4]
...
################ COLLECTING DATA ################################
	...
	num = Int.(readdlm("synth_data/"*j*"_parsed.csv",','))
