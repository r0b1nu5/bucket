...
push!(nums,collect(Float64,readdlm("data/time_split/"*journal*"_00_$yi.txt",'\t')[2:end-2,2]))
for i in 1:ny
	push!(nums,collect(Float64,readdlm("data/time_split/"*journal*"_00_$((yi+i)%100).txt",'\t')[2:end-2,2]))
end
...
aut = readdlm("data/time_split/"*journal*"_00_$yi.txt",'\t')[2:end-2,1]


for i in 1:(ny-hor+1)
	numi = collect(Float64,readdlm("data/time_split/"*journal*"_00_$((yi+i-1)%100).txt",'\t')[2:end-2,2])
	auti = readdlm("data/time_split/"*journal*"_00_$((yi+i-1)%100).txt",'\t')[2:end-2,1]
