...
for i in 1:length(ls[:,1])
	l = ls[i,:]
	PyPlot.plot([i2x[l[1]],i2x[l[2]]],[i2y[l[1]],i2y[l[2]]],"-k")
end
for i in 1:length(ids)
	PyPlot.plot(i2x[ids[i]],i2y[ids[i]],"ok")
#	PyPlot.text(i2x[ids[i]],i2y[ids[i]],"$(ids[i])")
	PyPlot.text(i2x[ids[i]],i2y[ids[i]],names[i])
end
# =#
